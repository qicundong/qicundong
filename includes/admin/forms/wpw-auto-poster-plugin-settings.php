	...
	<div class="sub-header">
		<div class="woo-slg-top-header-wrap">
			<div class="logo-header-wrap">
				...
				<!-- plugin name -->
				<div><?php esc_attr_e( 'Social Auto Poster Settings', 'wpwautoposter' ); ?></div>
			
			</div>
			<?php
			echo apply_filters(
			'wpweb_fb_settings_submit_button',
			'<form method="post" action="" class="wpw-auto-poster-posting-reset-setting-form-main">'.wp_nonce_field( 'wpw_auto_poster_reset_main_setting', 'reset_main_setting' ).'
		...
		<div class="woo-slg-top-error-wrap">
			...
			<?php
			if( isset( $_POST['wpw_auto_posting_reset_settings'] ) && $_POST['wpw_auto_posting_reset_settings'] == esc_html__( 'Reset All Settings', 'wpwautoposter' ) && wp_verify_nonce($_POST['reset_main_setting'], 'wpw_auto_poster_reset_main_setting') ) {
			...
			if( isset( $_GET['settings-updated'] ) ) {
				echo '<div id="message" class="updated fade notice notice-error is-dismissible"><p><strong>' . esc_html__( 'Changes Saved.', 'wpwautoposter' ) . '</strong></p></div>';
			}
			?>
		</div>
	</div>
